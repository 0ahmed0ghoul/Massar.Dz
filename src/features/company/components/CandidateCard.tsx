      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            <Badge variant="outline" className="border-white/20 text-foreground/80 text-xs">
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {candidate.skills.slice(0, 3).map(skill => (
              <span key={skill} className="text-xs bg-white/10 px-2 py-0.5 rounded-full text-foreground/60">{skill}</span>
            ))}
            {candidate.skills.length > 3 && <span className="text-xs text-foreground/40">+{candidate.skills.length - 3}</span>}
          </div>
        </div>
        {showScore && candidate.aiScore && (
          <div className="text-right">
            <span className="text-2xl font-bold text-[#639922]">{candidate.aiScore}%</span>
            <p className="text-xs text-foreground/40">AI Match</p>
          </div>
        )}
      </div>
    </Card>
