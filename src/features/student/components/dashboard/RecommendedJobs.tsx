        ...
        {displayJobs.map((job) => (
          <div key={job.id} className="group relative rounded-xl border border-white/[0.05] bg-gradient-to-r from-white/[0.04] to-transparent p-4 transition-all hover:border-[#639922]/30">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[15px] font-bold text-white transition-colors group-hover:text-[#639922]">{job.title}</p>
                <p className="text-xs text-white/50">{job.company} • {job.location || "Remote"}</p>
              </div>
            ...
            <div className="mt-4 flex gap-2">
              <button className="flex-1 rounded-lg bg-[#639922] py-2 text-xs font-bold text-white transition hover:bg-[#4f7a1a]">
                Quick Apply
              </button>
            </div>
          </div>
