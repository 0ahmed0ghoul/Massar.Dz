...
interface ApplyModalProps {
  ...
  jobTitle: string;
  companyName: string;
  onSubmit: (coverLetter: string, cvFile?: File) => Promise<boolean>;
  loading?: boolean;
}
...
export function ApplyModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onSubmit(coverLetter, cvFile || undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md border-white/10 bg-[#0f1117] text-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-[#639922]">
            Apply for {jobTitle}
          </DialogTitle>
