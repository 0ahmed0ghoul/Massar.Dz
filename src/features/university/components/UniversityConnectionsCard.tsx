    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Connected Students</h3>
          <span className="ml-auto text-sm text-white/40">{connectedStudents.length} total</span>
        </div>
        {connectedStudents.length === 0 ? (
          <p className="text-sm text-white/40">No connected students yet.</p>
        ) : (
            ...
            {connectedStudents.map((student) => (
              <div key={student.id} className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
                <div>
                  ...
                  <p className="text-xs text-white/40">{student.email} • {student.studentId}</p>
                </div>
      ...
      <div>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white">Pending Connection Requests</h3>
          <span className="ml-auto text-sm text-white/40">{pendingRequests.length} pending</span>
        </div>
              ...
              <div key={req.id} className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-xs text-white/30 mt-1">Sent: {new Date(req.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => setSelectedInvitation(req)} className="bg-[#639922] text-white">
                      <Eye className="mr-1 h-3 w-3" /> Verify
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => onReject(req.id, req.studentId)} className="border-white/20 text-white/80">
