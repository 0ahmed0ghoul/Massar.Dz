    ...
    <div className="relative min-h-screen bg-[#0b0c0e]">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="space-y-8">
          <div>
            <h1 className="text-2xl font-bold text-white sm:text-3xl">Connection Invitations</h1>
            <p className="text-sm text-white/40">Review, verify, and accept student connection requests.</p>
          </div>
                ...
                {pending.map((inv) => (
                  <div key={inv.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
                    <div>
                      ...
                      <p className="text-xs text-white/30 mt-1">Sent: {new Date(inv.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => setSelectedInvitation(inv)} className="bg-[#639922] text-white">
                        <Eye className="mr-1 h-3 w-3" /> Verify & Accept
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => rejectInvitation(inv.id, inv.studentId)} className="border-white/20 text-white/80">
                ...
                {history.map((inv) => (
                  <div key={inv.id} className="flex items-center justify-between rounded-xl border border-white/[0.08] p-3">
                    <div>
                      ...
                      <p className="text-xs text-white/40">{inv.profileData?.email || inv.profileData?.studentId}</p>
                    </div>
