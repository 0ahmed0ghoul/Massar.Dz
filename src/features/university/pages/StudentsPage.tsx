        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-foreground/40 sm:text-base">Manage student records, verify profiles, and issue certificates.</p>
            </div>
            <div>
              <input type="file" accept=".xlsx, .xls" onChange={handleFileUpload} className="hidden" id="excel-upload" />
              <label htmlFor="excel-upload">
                <Button variant="outline" disabled={uploading} className="border-white/[0.15] bg-white/[0.03] text-foreground hover:bg-white/[0.08]">
          ...
          <div className="group rounded-2xl border border-white/[0.09] bg-white/[0.03] backdrop-blur-md">
            <div className="p-5 sm:p-6">
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="ml-auto text-sm text-foreground/40">{students.length} total</span>
              </div>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-white/[0.08] text-left text-xs font-medium text-foreground/50">
                  ...
                  <tbody className="divide-y divide-white/[0.05]">
                    {students.map((student) => (
                      <tr key={student.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="py-3 pr-4 text-sm text-foreground/60">{student.studentId}</td>
                        <td className="py-3 pr-4 font-medium text-foreground">{student.firstName} {student.lastName}</td>
                        <td className="py-3 pr-4 text-sm text-foreground/60">{student.email}</td>
                        <td className="py-3 pr-4 text-sm text-foreground/60">{student.speciality}</td>
                        ...
                        <td className="py-3">
                          <div className="flex gap-2">
                            ...
                            {student.graduationEligible && student.status !== 'graduated' && (
                              <Button size="sm" onClick={() => handleClaimCertificate(student.id, `${student.firstName} ${student.lastName}`)} className="bg-[#639922] text-foreground text-xs">
                    ...
                    {students.length === 0 && (
                      <tr>
                        <td colSpan={7} className="py-8 text-center text-foreground/40">No students yet. Import an XLSX file.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                  ...
                  <div key={student.id} className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-foreground/40">{student.email}</p>
                        <p className="text-xs text-foreground/40 mt-1">ID: {student.studentId}</p>
                      </div>
                      {!student.isProfileVerified && (
                        <Button size="sm" variant="outline" onClick={() => verifyStudentProfile(student.id)} className="h-7 text-xs">Verify</Button>
                      )}
                    </div>
                    <div className="mt-2 text-xs text-foreground/60">Speciality: {student.speciality}</div>
                    <div className="mt-2 flex flex-wrap gap-2 justify-between items-center">
                      <span className={`text-xs ${student.connectionStatus === 'connected' ? 'text-[#639922]' : 'text-foreground/40'}`}>
