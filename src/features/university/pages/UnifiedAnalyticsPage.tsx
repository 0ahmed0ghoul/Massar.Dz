...
export default function UnifiedAnalyticsPage() {
  ...
  const universityName =
    profile?.university_name || decodeURIComponent(params.universityName || "");
  ...
  const [allDepartments, setAllDepartments] = useState<
    { department: string; count: number; hasConnection: boolean }[]
  >([]);
  ...
  const { registeredStudents } = useUniversityStudentConnection(profile);
  const { specialities } = useUniversitySpecialities(
    isRectorate ? universityName : ""
  );
      ...
      try {
        if (isRectorate) {
          const [students, graduates, depts] = await Promise.all([
            universityService.getTotalConnectedStudents(
          ...
          const overview = await universityService.getDepartmentOverview(
            profile.id,
            departmentName
          );
          ...
          console.log(
            "🎓 Graduated students in department:",
            gradStudents?.length
          );

          if (gradStudents && gradStudents.length > 0) {
            const studentIds = gradStudents.map((s) => s.id);
            const responses = await universityService.getStudentResponses(
              studentIds
            );
            console.log(
              "📊 Responses:",
            ...
            const detailData = universityService.calculateDetailedStats(
              responses,
              studentIds.length
            );
    ...
    const fetchSpecialityStats = async () => {
      ...
      try {
        const [chartData, detailData] = await Promise.all([
          universityService.getDepartmentCharts(
            universityName,
            specialityParam
          ),
          universityService.getDetailedStatistics(
            universityName,
            specialityParam
          ),
        ]);
    ...
    <div className="space-y-6">
      ...
      {details && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatCard
            icon={<Users className="h-4 w-4 text-[#639922]" />}
            label="Response Rate"
            value={`${details.responseRate.toFixed(1)}%`}
            sub={`${details.totalRespondents} of ${details.totalStudents}`}
          />
          <StatCard
            icon={<Briefcase className="h-4 w-4 text-blue-500" />}
            label="Employed"
            value={details.employedCount.toString()}
            sub={
              details.totalRespondents > 0
                ? `${(
                    (details.employedCount / details.totalRespondents) *
                    100
                  ).toFixed(1)}%`
                : "No data"
            }
          />
          <StatCard
            icon={<DollarSign className="h-4 w-4 text-[#e2c245]" />}
            label="Avg. Salary"
            value={`${details.averageSalary.toLocaleString()}DZ`}
            sub={`Median: ${details.medianSalary.toLocaleString()}DZ`}
          />
          <StatCard
            icon={<Target className="h-4 w-4 text-purple-500" />}
            label="Job Seeking"
            value={details.jobSearchingCount.toString()}
            sub={`${details.studyingCount} studying`}
          />
        </div>
      )}
      ...
      {charts && charts.situation.length > 0 && (
        <ChartCard
          ...
          subtitle="Current status of graduates"
          icon={<PieChartIcon className="h-5 w-5" />}
        >
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={charts.situation}
                dataKey="count"
                nameKey="answer"
                cx="50%"
                cy="50%"
                outerRadius={100}
                label={({ answer, count }) =>
                  `${answer.split("(")[0].trim()}: ${count}`
                }
              >
                {charts.situation.map((_, idx) => (
      ...
      {charts && charts.functions.length > 0 && (
        ...
        >
          <ResponsiveContainer
            width="100%"
            height={Math.max(200, charts.functions.length * 40)}
          >
            <BarChart
              data={charts.functions}
              layout="vertical"
              margin={{ left: 200 }}
            >
      ...
      {/* Empty State */}
      {(!charts ||
        (charts.situation.length === 0 && charts.sectors.length === 0)) && (
        <Card>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">{specialityParam}</h1>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Connected Students</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Graduates</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="flex gap-4 items-center">
          ...
          <Button
            variant={
              connectionFilter === "not_connected" ? "default" : "outline"
            }
            size="sm"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-2 px-4">Speciality</th>
                  <th className="text-left py-2 px-4">Students</th>
                  <th className="text-left py-2 px-4">Connection</th>
                  <th className="text-left py-2 px-4">Employed</th>
                  <th className="text-left py-2 px-4">Avg Salary</th>
                  <th className="text-left py-2 px-4">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredDepartments.map((dept) => {
                  const s = specialities.find(
                    (sp) => sp.speciality === dept.department
                  );
                  return (
                    <tr
                      key={dept.department}
                    ...
                    >
                      ...
                      <td className="py-2 px-4">
                        ...
                        >
                          {dept.hasConnection ? "Connected" : "Not Connected"}
                        </Badge>
                      </td>
                      <td className="py-2 px-4">{s?.employedCount || "-"}</td>
                      <td className="py-2 px-4">
                        {s ? `${s.averageSalary.toLocaleString()}DZ` : "-"}
                      </td>
                      <td className="py-2 px-4">
                        <Button
                          variant="ghost"
                          ...
                          onClick={() =>
                            navigate(
                              `/university/analytics/${encodeURIComponent(
                                universityName
                              )}/${encodeURIComponent(dept.department)}`
                            )
                          }
                        >
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          />
          <StatCard
            icon={<Briefcase className="h-4 w-4 text-[#e2c245]" />}
            label="Employed"
            value={details?.employedCount?.toString() || "-"}
          />
          <StatCard
            ...
            label="Response Rate"
            value={details ? `${details.responseRate.toFixed(1)}%` : "-"}
          />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          >
            <Card className="cursor-pointer hover:bg-white/[0.05]">
              <CardContent className="pt-6 flex items-center gap-3">
                <BarChart3 className="h-8 w-8 text-[#639922]" />
                <div>
                  <p className="font-semibold">View Speciality Details</p>
                  <p className="text-sm text-foreground/40">
                    Individual speciality statistics
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardContent className="pt-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-foreground/40">
                  {totalStudents} connected • {totalGraduates} graduates
                </p>
              </div>
            </CardContent>
          </Card>
...
function SalaryTable({
  ...
  data: { range: string; count: number; avgSalary: number }[];
}) {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-white/10">
            ...
            <th className="text-left py-2 px-4">Avg</th>
          ...
          {data.map((item, idx) => {
            ...
            return (
              <tr key={idx} className="border-b border-white/5">
                ...
                <td className="py-2 px-4">
                  {item.avgSalary.toLocaleString()}DZ
                </td>
                <td className="py-2 px-4">
                  <div className="flex items-center gap-2">
                    <div className="bg-white/10 rounded-full h-2 w-20">
                      <div
