      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <div className="space-y-6 sm:space-y-8">
          <div>
            ...
            <p className="text-sm text-white/40 sm:text-base">Manage connections, track student outcomes.</p>
          </div>
            ...
            {stats.map((s) => (
              <div key={s.label} className="group relative overflow-hidden rounded-2xl border border-white/[0.09] bg-white/[0.03] p-5 backdrop-blur-md transition-all hover:border-[#639922]/30">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium uppercase tracking-wider text-white/50">{s.label}</p>
          ...
          <div className="group rounded-2xl border border-white/[0.09] bg-white/[0.03] backdrop-blur-md transition-all hover:border-[#639922]/30">
            <div className="p-5 sm:p-6">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-white">Pending Connection Requests</h2>
                ...
                <div className="space-y-3">
                  {pendingInvitations.slice(0, 3).map((inv) => (
                    <div key={inv.id} className="flex flex-col gap-2 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="font-medium text-white">{inv.profileData?.firstName} {inv.profileData?.lastName}</p>
                        <p className="text-xs text-white/40">{inv.profileData?.email} • {inv.profileData?.speciality}</p>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => setSelectedInvitation(inv)} className="bg-[#639922] text-white hover:bg-[#4f7a1a] text-xs">
                          Verify & Accept
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => rejectInvitation(inv.id, inv.studentId)} className="border-white/20 text-white/80 hover:bg-white/10">
                          Reject
                        </Button>
                      </div>
                    </div>
                  ...
                  {pendingInvitations.length > 3 && (
                    <p className="text-center text-xs text-white/40">+ {pendingInvitations.length - 3} more</p>
                  )}
                </div>
              )}
            </div>
                  ...
                  {recentPlacements.map((p) => (
                    <div key={p.name} className="flex flex-col gap-2 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-xs text-white/40">{p.field} → {p.outcome} at {p.company}</p>
                      </div>
                  ...
                  <div key={s.id} className="flex flex-col gap-2 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 transition-all hover:border-[#639922]/30 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className="text-white/60 hover:text-[#639922]"
                      >
