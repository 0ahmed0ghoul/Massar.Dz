  ...
  return (
    ...
    >
      ...
      <div
        ...
        ${
          dragging
            ? "border-[#639922]/40 bg-[#639922]/15"
            : "border-white/10 bg-white/[0.04] group-hover:border-[#639922]/30 group-hover:bg-[#639922]/10"
        }`}
      >
        {uploading ? (
          ...
          <FileSpreadsheet
            className={`h-6 w-6 transition-colors ${
              dragging
                ? "text-[#639922]"
                : "text-white/30 group-hover:text-[#639922]"
            }`}
          />
        )}
      </div>
      <div>
        ...
        <p className="text-[11px] text-white/30 mt-1">
          Supports .xlsx and .xls — matches columns: id_card, full name,
          speciality type, academic year, group, section
        </p>
      </div>
      {fileName && !uploading && (
...
export default function StudentsPage() {
  ...
  const deleteRow = (i: number) =>
    setPreviewData((p) => p.filter((_, idx) => idx !== i));
  const startEdit = (i: number) => {
    setEditingRow(i);
    setEditForm({ ...previewData[i] });
  };
  ...
  const fieldChange = (k: string, v: string) =>
  ...
  const handleAccept = async (s: any) => {
    const m = matchResults.get(s.id);
    if (m) await acceptStudent(s, m);
  };
  const openRejectDialog = (s: any) => {
    setCurrentStudent(s);
    setRejectReason("");
  ...
  const filteredRegistered = registeredStudents.filter(
    (s) =>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#639922]">
                University Admin
              </span>
            </div>
            ...
            <p className="mt-1.5 text-sm text-white/35 max-w-xl">
              Upload your official student roster, then review and verify
              connection requests from registered students.
            </p>
          </div>
        ...
        <div className="flex items-center gap-2">
          <TabBtn
            ...
            icon={Users}
            label="Verification & Invitations"
            count={pendingCount}
          />
          <TabBtn
              ...
              <div className="flex items-center gap-3 border-b border-white/[0.06] px-6 py-4 bg-white/[0.02]">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-white/30">
                    Students who completed their profile and requested
                    department connection
                  </p>
                </div>
                <span className="ml-auto text-[11px] text-white/25 font-mono">
                  {filteredRegistered.length} results
                </span>
              </div>
                  ...
                  <thead>
                    <tr className="border-b border-white/[0.05]">
                      {[
                        ...
                        <th
                          key={i}
                          className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-widest text-white/25 bg-white/[0.015]"
                        >
                          {col}
                        </th>
                      ))}
                      ...
                      filteredRegistered.map((student) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-5 py-3.5 text-[12px] text-white/60">
                              {student.speciality}
                            </td>
                            <td className="px-5 py-3.5 text-[12px] text-white/50">
                              {student.speciality_type}
                            </td>
                            <td className="px-5 py-3.5">
                                ...
                                {student.university_connection_status === "rejected" &&
                                  student.rejection_reason && (
                                    <span
                                      ...
                                      title={student.rejection_reason}
                                    >
                                      {student.rejection_reason}
                                    </span>
                                  )}
              ...
              {filteredRegistered.length > 0 && (
                <div className="border-t border-white/[0.04] px-6 py-3">
                  <p className="text-[11px] text-white/20">
                    {filteredRegistered.length} student
                    {filteredRegistered.length !== 1 ? "s" : ""}
                  </p>
                </div>
              )}
            </div>
        ...
        {activeTab === "database" && (
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            ...
            <div className="flex items-center gap-3 border-b border-white/[0.06] px-6 py-4 bg-white/[0.02]">
              ...
              <div>
                <p className="text-sm font-semibold text-white/80">
                  Official University Roster
                </p>
                <p className="text-[11px] text-white/30">
                  Imported from your XLSX file — used as the reference for
                  student verification
                </p>
              </div>
              <span className="ml-auto text-[11px] text-white/25 font-mono">
                {officialStudents.length} records
              </span>
            </div>
                ...
                <tbody>
                  {officialStudents.length === 0 ? (
                        ...
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-white/20 mt-1">
                              Use the upload area above to import your official
                              XLSX file
                            </p>
                          </div>
                        </div>
                    ...
                    officialStudents.map((s, i) => (
                      ...
                      >
                        <td className="px-5 py-3.5 font-mono text-[12px] text-white/50">
                          {s.student_id}
                        </td>
            ...
            {officialStudents.length > 0 && (
              <div className="border-t border-white/[0.04] px-6 py-3">
                <p className="text-[11px] text-white/20">
                  {officialStudents.length} record
                  {officialStudents.length !== 1 ? "s" : ""} imported
                </p>
              </div>
            )}
          </div>
        ...
        <DialogContent className="border-white/[0.09] bg-[#0f1117] max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/40 text-[13px]">
              Write a clear reason. The student will receive this via email and
              can update their profile and re-apply.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex items-start gap-4 border-b border-white/[0.07] px-6 py-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] text-white/35 mt-0.5">
                Edit or remove rows before confirming. Duplicate student IDs
            ...
            <table className="w-full min-w-[700px] border-collapse text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="border-b border-white/[0.07] bg-[#0f1117]">
                  ...
                  {PREVIEW_COLS.map((c) => (
                    <th
                      key={c.key}
                      className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-widest text-white/25"
                    >
                      {c.label}
                    </th>
                  ))}
              ...
              <tbody>
                {previewData.map((row, idx) => (
                  ...
                  >
                    <td className="px-2 py-2.5 text-[11px] text-white/20 font-mono">
                      {idx + 1}
                    </td>
                    {editingRow === idx ? (
                      <>
                        {PREVIEW_COLS.map((c) => (
                          <td key={c.key} className="px-2 py-1.5">
                            <input
                              ...
                              onChange={(e) =>
                                fieldChange(c.key, e.target.value)
                              }
                        ...
                        {PREVIEW_COLS.map((c) => (
                          <td
                            ...
                            className={`px-3 py-2.5 ${
                              c.key === "student_id"
                                ? "font-mono text-[12px] text-white/50"
