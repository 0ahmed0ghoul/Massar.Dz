              ...
              <div className="flex cursor-pointer items-start gap-3 flex-1" onClick={() => navigate(`/dashboard/admin/pending/${profile.id}`, { state: { profile } })}>
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="font-medium text-foreground">{isUniversity ? profile.university_name : profile.company_name}</p>
                    <span className="rounded-full bg-amber-400/20 px-2 py-0.5 text-xs font-medium text-amber-400">Pending</span>
                  </div>
                  <p className="mt-0.5 text-xs text-foreground/40">{profile.first_name} {profile.last_name} • {profile.email}</p>
                  <p className="mt-1 text-xs text-foreground/30">Submitted: {profile.completed_at ? new Date(profile.completed_at).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => navigate(`/dashboard/admin/pending/${profile.id}`, { state: { profile } })} className="rounded-lg border border-white/20 p-2 text-foreground/60 transition hover:bg-white/10 hover:text-foreground"><Eye className="h-4 w-4" /></button>
